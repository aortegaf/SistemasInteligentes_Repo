...
var CanvasController = {
  ...
  getImage: () => {
    ...
    var canvasImg = CanvasController.canvas
      .getContext("2d")
      .getImageData(0, 0, canvasRect.width, canvasRect.height);
    var exit = [];
  ...
  drawImg: (img) => {
    ...
    tmpEl
      .getContext("2d")
      .putImageData(CanvasController.imgToImgData(img), 0, 0);
  },
  ...
  getImgOfCell: (img, x, y) => {
    ...
    for (var i = 0; i < cellSize; i++) {
      ...
      for (var j = 0; j < cellSize; j++) {
        if (img[x * cellSize + i] && img[x * cellSize + i][y * cellSize + j])
          tmp.push(img[x * cellSize + i][y * cellSize + j]);
   ...
   */
  printCell: (x, y) => {
    CanvasController.drawImg(
      CanvasController.getImgOfCell(CanvasController.getImage(), x, y)
    );
  },
  ...
  DrawMatrix: (matrix) => {
    ...
    for (var i = 0; i < matrix.length * cellSize; i++) {
      var tmp = [];
      for (var j = 0; j < matrix[0].length * cellSize; j++)
        tmp.push({ r: 0, g: 0, b: 0, t: 255 });
      imgMatrix.push(tmp);
    }
    ...
    for (var i = 0; i < matrix.length; i++) {
      ...
      for (var j = 0; j < matrix[i].length; j++) {
        ...
        switch (matrix[i][j]) {
          case TYPES.CAR:
            toPush = { r: 255, g: 0, b: 0, t: 255 };
            break;
          case TYPES.HOUSE:
            toPush = { r: 255, g: 255, b: 0, t: 255 };
            break;
          case TYPES.OWL:
            toPush = { r: 0, g: 255, b: 255, t: 255 };
            break;
          case TYPES.STREET:
...
var MyAgent = {
  ...
  isStreet: (cell, x, y) => {
    ...
    for (var i = 0; i < cell.length; i++) {
      for (var j = 0; j < cell[i].length; j++) {
        if (
          MyAgent.canvasController.equalPixels(
            cell[i][j],
            MyAgent.StreetColor,
            15
          )
  ...
  isCar: (cell, x, y) => {
    ...
    for (var i = 0; i < cell.length; i++) {
      for (var j = 0; j < cell[i].length; j++) {
        if (
          MyAgent.canvasController.equalPixels(cell[i][j], MyAgent.CarColor, 0)
        ) {
          bool = true;
        }
      }
    ...
    for (var i = 0; i < cell.length; i++) {
      for (var j = 0; j < cell[i].length; j++) {
        if (
          ...
          MyAgent.canvasController.equalPixels(cell[i][j], MyAgent.OwlColor, 15)
        ) {
          bool = true;
        }
      }
  ...
  getMatrix: () => {
    ...
    for (var i = 0; i < MyAgent.pixelMatrix.length / cellSize; i++) {
      ...
      for (
        var j = 0;
        j < MyAgent.pixelMatrix[i * cellSize].length / cellSize;
        j++
      ) {
        var subImg = MyAgent.canvasController.getImgOfCell(
          MyAgent.pixelMatrix,
          i,
          j
        );
